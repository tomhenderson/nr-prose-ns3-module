...
def plot_flow_statistics(all_stats_by_hops, eval_names, output_path, output_file_name):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, output_file_name))
    plt.close()  # Close the plot
...
def calculate_and_plot_combined_statistics(metrics, all_eval_data, eval_names, output_file_name, campaignLabel):
    fig, axs = plt.subplots(2, 5, figsize=(20, 10))
    ...
    for i, metric in enumerate(metrics):
        ...
        for j, eval_data in enumerate(all_eval_data):
            ...
            axs[row_pos, col_pos].text(j-0.15, mean, f'{mean:.2f}', ha='center', va='bottom', rotation=90)
        ...
        with open(output_filename, mode='w', newline='') as outfile:
            ...
            for row in reader:
                if int(row["totalnTxCtrl"]) > 0 and int(row["totalnTxData"]) > 0:
                    ...
                    # Check if success rates for control or data are less than 100
...
def main(evalData, campaignName, campaignLabel):
    ...
    combined_metrics_filename = f"{campaignName}_combined_evalSimStats.png"
    combined_statistics_filename = f"{campaignName}_combined_evalSimStats_meanCI.png"
    combined_flow_stats_filename = f"{campaignName}_combined_flowStats.png"

    plot_combined_metrics(metrics, all_eval_data, eval_names, combined_metrics_filename, campaignLabel)
    calculate_and_plot_combined_statistics(metrics, all_eval_data, eval_names, combined_statistics_filename, campaignLabel)
    plot_flow_statistics(all_stats_by_hops, eval_names, "", combined_flow_stats_filename)

    phy_combined_statistics_filename = f"{campaignName}_combined_evalSimPhyStats_meanCI.png"
    calculate_and_plot_combined_statistics(phy_metrics, phy_all_eval_data, eval_names, phy_combined_statistics_filename, campaignLabel)
