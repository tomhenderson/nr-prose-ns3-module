...
class NrSlUeProseDirLinkContext : public Object
{
  public:
    ...
    NrSlUeProseDirLnkSapProvider*
        m_nrSlUeProseDirLnkSapProvider; ///< The pointer to the SAP in which the direct link
                                        ///< receives calls from the ProSe layer
    NrSlUeProseDirLnkSapUser::DirectLinkIpInfo
        m_ipInfo; ///< The IP addresses used by the peers in the link
    bool
        m_hasPendingSlDrb; ///< Flag to indicate if the UE is having a SL-DRB pending for activation
...
class NrSlUeProse : public NrSlUeService
{
  ...
  public:
     ...
     */
    typedef std::unordered_map<uint32_t, Ptr<NrSlUeProseDirLinkContext>>
        NrSlDirectLinkContextMapPerPeerL2Id;
    ...
    /**
     * \brief Add an L3 UE-to-Network relay service that the UE in the role of relay UE will
     * provide.
     *
     ...
     */
    void AddL3U2nRelayServiceConfiguration(uint32_t relayServiceCode,
     ...
     *
     */
    typedef void (*RelayDiscoveryTracedCallback)(uint32_t remoteL2Id,
                                                 uint32_t relayL2Id,
                                                 uint32_t relayCode,
                                                 double rsrpValue);
     ...
     * \param [in] rsrpValue RSRP value in dBm
     */
    typedef void (*RelaySelectionTracedCallback)(uint32_t remoteL2Id,
     ...
     * \param [in] rsrpValue RSRP value in dBm
     */
    typedef void (*RelayRsrpTracedCallback)(uint32_t remoteL2Id,
                                            uint32_t currentRelayL2Id,
                                            double rsrpValue);
     ...
     */
    void AddRelayDiscovery(uint32_t relayCode,
                           uint32_t dstL2Id,
  ...
  private:
    ...
    NrSlL3U2nRelayServices
        m_l3U2nRelayProvidedSvcs; ///< UE-to-Network Relay services provided by the UE
     ...
     * \param slInfo the parameters to be used for the sidelink data radio bearer
     */
    void ConfigureDataRadioBearersForU2nRelay(
        uint32_t peerL2Id,
     ...
     * \param ipInfo the IP configuration to use
     */
    void RemoveDataRadioBearersForU2nRelay(uint32_t peerL2Id,
                                           NrSlUeProseDirLnkSapUser::DirectLinkRelayInfo relayInfo,
                                           NrSlUeProseDirLnkSapUser::DirectLinkIpInfo ipInfo);

    /**
     *  Locate the RSRP measurement corresponding to the L2 ID
     *  return a pair <RSRP value, whether or not the relay passed the threshold/hysteris criteria>
     * if it exists or a pair <-inf,false> if not
     *
    ...
    /**
     * Update the list of discovered relays and their related parameters (L2 ID, service code, RSRP,
     * eligibility after L3 filtering)
     *
